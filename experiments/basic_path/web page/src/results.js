...
const ExpResults = (props) => {
  ...
  useEffect(() => {
    const fetchPrediction = async () => {
      try {
        await pushkin.connect("/api/basic_path");

        const response = await pushkin.customApiGet('/getExperimentData', { params: { userID: props.userID } });
        const keyPressData = response.data.key_press;

        const predictionResponse = await pushkin.customApiPost('/getPrediction', { key_press: keyPressData });
  ...
  return (
    <Container className="mt-5 text-center">
      ...
      <div>
        <h1>Your Result</h1>
        {prediction ? (
          <p>Your predicted handedness is: <strong>{prediction}</strong></p>
        ) : (
